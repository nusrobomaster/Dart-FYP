...
enum {
    BRITERENCODER_COUNTS_PER_REV = 65536U
};

static const float briterencoder_rad_per_count = 6.2831853071795864769f / (float)BRITERENCODER_COUNTS_PER_REV;
static const float briterencoder_deg_per_count = 360.0f / (float)BRITERENCODER_COUNTS_PER_REV;
...
float briterencoder_u32_to_rad(uint32_t raw)
{
    return (float)(raw & 0xFFFFU) * briterencoder_rad_per_count;
}

float briterencoder_u32_to_deg(uint32_t raw)
{
    return (float)(raw & 0xFFFFU) * briterencoder_deg_per_count;
}
