...
//  ui_init1();
  ...
  myQueue_screenHandle = osMessageQueueNew (16, sizeof(float), &myQueue_screen_attributes);
...
//  MX_USB_DEVICE_Init();
  ...
  for(;;)
  {
	  osStatus_t st = osMessageQueueGet(myQueue_screenHandle, &w, NULL, osWaitForever);
	  if (st == osOK) {
		  char buf[20];
		  sprintf(buf, "Weight: %.2f g", w);
		  Displ_WString(160, 240, buf, Font12, 1, RED, WHITE); // your handler
	  }
  }
  /* USER CODE END 5 */
}
...
//		hx.ready = false;
	weight =  hx711_get_units(&hx, 10);
	 osStatus_t st = osMessageQueuePut(myQueue_screenHandle, &weight, 0, 0);
//	}
    osDelay(10);
  }
